...
import { generateTransactionId, validateMpesaCallback, parseMpesaCallback } from '../utils/mpesaUtils.js';
...
router.post('/initiate', [
  ...
  body('customerName').optional().isString().trim().isLength({ max: 100 })
], async (req, res) => {
...
router.post('/callback', async (req, res) => {
  try {
    ...
    if (!purchaseLog) {
      logger.error('Purchase log not found for callback:', parsedData.checkoutRequestId);
      return res.status(404).json({ 
        success: false, 
    ...
    if (parsedData.resultCode === 0) {
      ...
      purchaseLog.mpesaReceiptNumber = parsedData.callbackMetadata.mpesaReceiptNumber;
      
      // Generate download token
      const downloadToken = generateDownloadToken(purchaseLog._id, purchaseLog.bookId._id);
...
router.post('/retry-delivery/:transactionId', async (req, res) => {
  try {
    const { transactionId } = req.params;
    ...
    res.json({
      ...
      message: whatsappResult.success ? 'Delivery retried successfully' : 'Delivery retry failed',
